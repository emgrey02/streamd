...
interface SmallCreditsListProps {
    creds: CastCrewItem[];
    contentId?: string;
    contentType: string;
    personId?: string;
 ...
 * this uses the BigCards
 */
export default function SmallCreditsList({
    creds,
    contentId,
    contentType,
    personId,
}: SmallCreditsListProps) {
    return (
        <div className={`flex flex-col`}>
                    ...
                    >
                        {creds.map(
                            (p: CastCrewItem, index: number) =>
                                index < 4 && (
                                    <li key={index}>
                                        <BigCard data={p} type={contentType} />
                                    </li>
                                )
                        )}
                    </ul>
            ...
            <Link
                ...
                href={`${contentId || personId}/credits`}
            >
                See All Credits
            </Link>
        </div>
